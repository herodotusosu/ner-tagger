...
for i in range(0, len(LeftOverUnannotated)):
	if '@probability' in LeftOverPred[i+more]:
				...
				if unkCount <= unkCountLimit:
					for word2 in sent:
						...
						for p in TestUNKs:
							if word.lower() in TestUNKs[p]:
								mp = word2.split('$$$')[1]
								mpSum = float(mp)+float(TestUNKs2MPs[word.lower()])
								origUNKs = sentID2origUNKcounts[sentID]
								freq = TestUNKs[p][word.lower()]
								list = [float(p), float(freq), float(mpSum), float(origUNKs), (float(sentProb) / (len(sentList))), sentList]
								key = word.lower()
		...
		wordID = -1
		more += 1
	if len(LeftOverUnannotated[i].split()) == 0:
		pass
	else:
