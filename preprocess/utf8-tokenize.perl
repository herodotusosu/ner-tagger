...
-f : French text
-z : Galician text
-s : Keep newlines the same as in the original, seperate tokens through spaces.
...
while (<>) {
    ...
    # replace newlines or tab characters with blanks
    tr/\n\t/  /;
	...
	if (/^<.*>$/) {
        ...
        # Do not split on newlines if we are only separating by spaces and not
        # newlines.
        if (defined($opt_s)) {
            @F = split(/ /);
        } else {
		...
		if (defined($Token{$_})) {
            print encode('utf8', "$_$sep$suffix");
		    next;
		}
