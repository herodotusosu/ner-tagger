...
echo LIMITING UNANNOTATED CORPUS TO ONLY SENTS CONTAINING CAPPED TEST UNKS
python harvestSentsWithTestUNKs.py LeftOverUnannotated.txt testUNKs.dat trainWords.dat > LeftOverUnannotated2.txt ### add a 4th sys.argv element if you want to truly find most informative without concern for ability to accurately tag it.
...
do
	...
	python ../GenPredTestSentStats.py LeftOverUnannotated.ftrs sentID2origUNKcounts.dat predUnannotated$beta.txt predStats$beta.txt testUNKs.dat test$beta.ftrs train$beta.ftrs False #7 #This last variable represents the most allowable number of UNK words in a sentence
	### Pickles out sentID2tUNK2listStats.dat
	rm predUnannotated$beta.txt
	rm predStats$beta.txt
	
	echo IDENTIFYING NEWLY TAGGED SENTS WITH TEST UNKS WHICH HAVE THE HIGHEST CONFIDENCE	
	...
	python getInformativeSents.py sentID2tUNK2listStats.dat LeftOverUnannotated.ftrs $TrainLength $recursivelyAddedPortion sentID2origUNKcounts.dat > LeftOverUnannotated2.ftrs
	...
	then
		...
		#cp train$newBeta.ftrs DeBug/trainNew$beta.ftrs
