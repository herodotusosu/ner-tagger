...
THE LAST NUMBER LISTED WILL GET HIHGEST PRIORITY IN SORTING
 """		
allLists = sorted(sorted(sorted(sorted(sorted(allLists, key = lambda x : x[0], reverse = True), key = lambda x : x[3], reverse = True), key = lambda x : x[2], reverse = True), key = lambda x : x[1], reverse = True), key = lambda x : x[4], reverse = True)
