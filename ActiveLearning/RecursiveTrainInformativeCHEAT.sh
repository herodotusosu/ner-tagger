...
python harvestSentsWithTestUNKsCHEAT.py test$beta.ftrs testUNKs.dat trainWords.dat -normal > LeftOver.ftrs ### add a 4th sys.argv element if you want to truly find most informative without concern for ability to accurately tag it.
...
unImportantLength=$(cat unImportantTestSet.ftrs | sed '/^\s*$/d' | wc -l)
echo LIMITED LENGTH IS
cat LeftOver.ftrs | sed '/^\s*$/d' | wc -l
...
orderedLength=$(cat Results/orderedTestSents.ftrs | sed '/^\s*$/d' | wc -l)
...
while [ $testLength -gt $(($testLength - $orderedLength+$sentsAtATime)) ]
do
	python getBestSents.py newTest.ftrs $sentsAtATime -train > delete.txt
	cat added2train.ftrs delete.txt > added2train2.txt
	mv added2train2.txt added2train.ftrs
	python getBestSents.py newTest.ftrs $sentsAtATime -leftOver > newTest2.ftrs
	mv newTest2.ftrs newTest.ftrs
	delLength=$(cat delete.txt | sed '/^\s*$/d' | wc -l)
	
	echo REMOVING SENTS FROM TEST FILE
	python removeExtractedTestSents.py test$beta.ftrs delete.txt > test$beta.ftrs2
	mv test$beta.ftrs2 test$beta.ftrs	
	rm delete.txt
	testLength=$(cat test$beta.ftrs | sed '/^\s*$/d' | wc -l)
